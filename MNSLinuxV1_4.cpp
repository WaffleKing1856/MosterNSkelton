...
\ \ / /             | ___ \                  | |              | |
 \ V / ___   _   _  | |_/ / ___   __ _   ___ | |__    ___   __| |
  ...
  \_/ \___/  \__,_| \_| \_|\___| \__,_| \___||_| |_| \___| \__,_|
            ...
            if(this->Class == "Fighter" || this->Class == "Thief"){
				this->str = 100;
			}
            }
        }
...
void giveSkelly(Character* PC){
	PC->setSkellyKey(true);
	inventory.push_back("Skeleton Key");
}
void giveMap(Character* PC){
	PC->setSkellyKey(true);
         ...
         \ 1) Attack the monster normally        /
          \ 2) Just let the monster attack you  /
           ...
           //Player normal attack
            if(playerChoice == "1"){
				float damage = (rand() % PC->getStr()) - ((rand() % def)-4);
                if((rand() % 101) > 15){
                    hp -= damage;
            ...
            else if (playerChoice == "3"){ //Spell Casting
                damage = PC->Spell(PC);
                
                if(damage != 0 && damage != 123456789 && damage != 987654321){
					combatPrint(hp, str, def, PC, "You Cast your spell sucessfully", enemyPicture);
					hp -= damage;
				}else if (damage == 987654321){
					system("clear");
					combatPrint(hp, str, def, PC, "", enemyPicture);
					continue;
				}else
					combatPrint(hp, str, def, PC, "You did not have enough mana", enemyPicture);
                if (hp <= 0) {
                   continue;
                }
           }
            else if (playerChoice == "4"){ //Run
            ...
            printWithDelay("The monster plots their attack \n");
            printDots(3);
            cout << "\n\n";
            system("clear");
            ...
            if(((rand() % 101) > 15)/PC->getLuck()){
				//hp -= (rand() % PC->getStr()) - (rand() % def);
                PC->decHP(rand() % str + 1);
                if(worldLevel == 1){
...
dietySays("Welp, you're on your own now, Good Luck \nAnd don't call if you need anything!\n\n");
 ...
 while(true){
    ...
    changeSpeed();
		...
		system("clear");
			while(true){
			printInColor("yellow", "\nhey big boi whatchu need\n\n1)EXP\n2)Skelly Key\n3)Map\n4)Increase any stat\n0)Exit\n>");
			cin >> hacks;
			if(hacks == "1"){
				cout << "\nEnter EXP amount\n>";
				cin >> tempInt;
				PC.expUp(tempInt);
				printInColor("yellow", "\nall set big boi, here you are\n\n");
			}
			else if(hacks == "2"){
				giveSkelly(&PC);
				printInColor("yellow", "\nall set big boi, here you are\n\n");
